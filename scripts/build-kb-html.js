...
(function () {
  ...
  function servicePtfLevelStatus(service, version, userLevel) {
    ...
    if (userLevel === null || isNaN(userLevel)) return { status: 'needs-ptf', minLevel: minLevel };
    return userLevel >= minLevel
      ? { status: 'available' }
      : { status: 'needs-ptf', minLevel: minLevel };
  }
  ...
  function svcReqLabel(service) {
    ...
    if (result.status === 'needs-ptf') {
      var levelText = result.minLevel !== undefined ? '需 ' + group + ' Level ' + result.minLevel + '+' : '需PTF';
